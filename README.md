...
Il piano automatico propone due gruppi e aggiunge un terzo solo quando migliora la copertura. Ogni gruppo ha il proprio podio con le tre migliori fasce di date. L'anteprima è visibile solo quando viene scritto `Konrad` nel campo del nome. Per renderla visibile a tutti, imposta `SHOW_MEETING_PLAN_TO_EVERYONE` su `true` in `components/cena-planner.tsx`.
